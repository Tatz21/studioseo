...
- **Directory Structure:** Group by feature domain (e.g., `crawler/`, `analyzer/`, `components/`, `api/`, `utils/`).
- **Filenames:**
  - Kebab-case or camelCase for utilities (`url-parser.ts` / `urlParser.ts`).
  - PascalCase for UI components (`AuditCard.tsx` / `MetricGauge.tsx`).
...
- [ ] Code builds without errors or unhandled warnings.
- [ ] Documentation (`README.md`, `MEMORY.md`, `DESIGN.md`) updated.
- [ ] UI rendered with rich aesthetic standards matching `DESIGN.md`.
- [ ] Error states tested (e.g., offline URL, timeout, malformed HTML).
